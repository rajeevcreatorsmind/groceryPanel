...
import { ArrowLeft, Upload, Link as LinkIcon, Loader2, X, Plus, Link } from '@/components/icons';
...
export default function AddProductPage() {
  ...
  // Image handlers (unchanged from your original)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      urlInputs.forEach((url) => {
        if (url.trim()) finalImageUrls.push(url.trim());
      });
        ...
        <div className="max-w-full mx-auto">
          <div className="flex items-center gap-4 mb-10">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">Create product with multiple pack sizes/variants</p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-3 border-t pt-10 flex justify-end gap-6">
                  <button
                    type="button"
                    onClick={() => router.back()}
                    className="px-10 py-4 border border-gray-300 rounded-xl text-lg font-medium hover:bg-gray-50 transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-10 py-4 bg-purple-600 text-white rounded-xl text-lg font-bold hover:bg-purple-700 disabled:opacity-60 flex items-center gap-3 transition"
                  >
                    {loading ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" />
