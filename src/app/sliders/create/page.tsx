...
export default function CreateSliderPage() {
  ...
  const [form, setForm] = useState({
    title: '',
    imageUrl: '',
    publishType: 'draft' as 'draft' | 'scheduled', // Two options: draft or scheduled
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'sliders'), {
        ...
        status: status,
        publishType: form.publishType, // Store publish type for reference
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-purple-400 transition">
                    {preview ? (
                      <div className="relative">
                        ...
                        <button type="button" onClick={() => { setPreview(''); setSelectedFile(null); setForm(prev => ({ ...prev, imageUrl: '' })); }}
                ...
                {imageSource === 'url' && (
                  <div className="space-y-4">
                    ...
                    {preview && (
                      <div className="relative">
                        <img src={preview} alt="Preview" className="max-h-64 mx-auto rounded-lg object-contain shadow" />
                        <button type="button" onClick={() => { setPreview(''); setForm(prev => ({ ...prev, imageUrl: '' })); }}
                          className="absolute top-2 right-2 p-1.5 bg-red-600 text-white rounded-full hover:bg-red-700">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
